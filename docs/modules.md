...
NovaGo explicitly abandons the centralized `node_modules` constraint. Instead, it features a **Decentralized Global Import Engine** allowing zero-configuration integration. Modules are resolved out of a global `~/.nova_libs/` cache.
...
```novascript
import { appVersion, formatData } from "./utils.ng"

print("Formatting v:", appVersion)
```

### 2. Node.js Native Core Modules (`go:`)
Direct mapping to legacy backend Node.js runtimes. Every built-in Node.js module is fully available natively!

```novascript
import { readdirSync, readFileSync } from "go:fs"
import { randomUUID, createHash } from "go:crypto"
import { platform, uptime } from "go:os"
import { execSync } from "go:child_process"
import { join, extname } from "go:path"
import { EventEmitter } from "go:events"
import { inspect } from "go:util"
...
### 3. NPM Packages (`npm:`)
Instantly resolves and fetches dependencies. If an `npm:` namespace dictates a package that has not yet been fetched, NovaGo stops, securely downloads it to the global cache, and seamlessly proceeds execution.
...
```novascript
import { utility } from "github:user/repo@v1.0.0/src/util.ng"
```
...
You can permanently cache and map utility aliases to your OS paths utilizing `novago install -g`.

```bash
# Globally registers Boxen.
# An executable shim `.cmd` or `.sh` is added to `~/.nova/bin`.
novago install -g npm:boxen
```
