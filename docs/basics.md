# NovaGo v5.5.5 Basics

NovaGo is a dynamically-typed, interpreted language heavily inspired by JavaScript/TypeScript but built with a cleaner, minimalistic aesthetic.

## Variables & Scopes
NovaGo supports multi-level scoping:
...
## Data Types
NovaGo supports standard literals:
- **Numbers**: `10`, `3.14`
...
## Operators
NovaGo v5.5.5 features a complete set of operators:
- **Arithmetic**: `+`, `-`, `*`, `/`, `%`, `**` (Power)
- **Comparison**: `==` (`is`), `!=` (`isnt`), `<`, `>`, `<=`, `>=`
- **Logical**: `&&` (`and`), `||` (`or`), `!` (`not`)
- **Assignment**: `=`, `+=`, `-=`
...
## Modules
NovaGo v5.5.0 has a modular file architecture via `include()`.
```javascript
// mathLib.ng
let api = "Math"
