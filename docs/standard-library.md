# NovaGo v5.5.5 Standard Library

NovaGo 5.5.0 brings a production-grade standard library to enable full-scale application development, including complex logical parsing, server handling, and data manipulation.
...
### `JSON` Module
- `JSON.stringify(val)`: Converts a NovaGo value to a JSON string.
- `JSON.parse(str)`: Parses a JSON string back into a NovaGo object/array/value (recursive).
...
- `Sys.eval(code)`: Dynamically evaluates a string of NovaGo code.
