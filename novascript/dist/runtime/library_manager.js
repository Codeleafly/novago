...
class LibraryManager {
    ...
    static ensureDirs() {
        ...
        if (!fs.existsSync(this.NPM_DIR))
            fs.mkdirSync(this.NPM_DIR, { recursive: true });
        if (!fs.existsSync(this.GITHUB_DIR))
            fs.mkdirSync(this.GITHUB_DIR, { recursive: true });
        if (!fs.existsSync(this.HTTPS_DIR))
            fs.mkdirSync(this.HTTPS_DIR, { recursive: true });
    }
    /**
     ...
     */
    static async resolve(modulePath) {
        this.ensureDirs();
    ...
    static async resolveNpm(pkg) {
        ...
        if (!fs.existsSync(pkgPath)) {
            console.log(chalk_1.default.blue(`📥 Downloading npm:${pkg}...`));
            try {
                ...
                (0, child_process_1.execSync)(`npm install ${pkg} --prefix "${this.NPM_DIR}" --no-save --quiet`, { stdio: "inherit" });
                console.log(chalk_1.default.green(`✔ Cached npm:${pkg}`));
            }
            catch (e) {
     ...
     */
    static async resolveGithub(repoPath) {
        // Format: user/repo[@version][/filepath]
        const match = repoPath.match(/^([^/@]+)\/([^/@]+)(?:@([^/]+))?(?:\/(.+))?$/);
        if (!match)
            throw new Error(`Invalid GitHub path: ${repoPath}. Expected user/repo[@version][/filepath]`);
        const user = match[1];
        const repo = match[2];
        ...
        if (!requestedFile) {
            ...
            try {
                if (!fs.existsSync(manifestPath)) {
                    console.log(chalk_1.default.blue(`🔍 Fetching manifest for github:${user}/${repo}...`));
        ...
        if (!fs.existsSync(mainFile)) {
            console.log(chalk_1.default.blue(`📥 Downloading github:${user}/${repo}/${requestedFile}...`));
            ...
            console.log(chalk_1.default.green(`✔ Cached github:${repoPath}`));
        }
    ...
    static async resolveHttps(url) {
        ...
        if (!fs.existsSync(targetFile)) {
            console.log(chalk_1.default.blue(`📥 Downloading ${url}...`));
            fs.mkdirSync(targetDir, { recursive: true });
            const content = await this.download(url);
            fs.writeFileSync(targetFile, content);
            console.log(chalk_1.default.green(`✔ Cached ${url}`));
        }
     ...
     */
    static async installGlobal(modulePath) {
        console.log(chalk_1.default.blue(`🚀 Installing global tool: ${modulePath}`));
        ...
        for (const [cmdName, binSource] of Object.entries(binsToInstall)) {
            const sourceFile = path.resolve(pkgDir, binSource);
            if (!fs.existsSync(sourceFile)) {
                console.log(chalk_1.default.yellow(`⚠ Warning: Bin script '${binSource}' not found in package. Skipping.`));
                continue;
            }
            ...
            console.log(chalk_1.default.green(`✔ Installed executable '${cmdName}' -> ${binDir}`));
        ...
        console.log(chalk_1.default.yellow(`Make sure ${binDir} is in your system PATH.`));
    }
    /**
    ...
    static clean() {
        if (fs.existsSync(this.BASE_DIR)) {
            ...
            console.log(chalk_1.default.green("✔ Global library cache cleared."));
        }
    }
}
exports.LibraryManager = LibraryManager;
LibraryManager.BASE_DIR = path.join(os.homedir(), ".nova_libs");
LibraryManager.NPM_DIR = path.join(LibraryManager.BASE_DIR, "npm");
LibraryManager.GITHUB_DIR = path.join(LibraryManager.BASE_DIR, "github");
LibraryManager.HTTPS_DIR = path.join(LibraryManager.BASE_DIR, "https");
